    ...
    <div class="border-b py-2 text-white" :class="'bg-[#ffaa69]'">
      <div class="mx-auto flex w-full max-w-[1300px] justify-between">
        <div class="flex-shrink-0 flex items-center" style="width: 257px;padding-left: 10px;">
          <a href="/dropshipping" class="text-white">
        ...
        <div class="w-full flex justify-center">
          <div class="w-full max-w-screen-sm bg-white p-2 flex rounded-md relative">
            <div class="w-full flex items-center">
              <input id="navInput" @focus="searchWord" @input="searchWord" @keyup.enter="search" ref="searchInput" v-model="text" class="w-full px-2 py-2 h-8 outline-none rounded-lg text-black" type="text" placeholder="Search USA Products ">
...
async function search() {
  const destination = await resolveSearchDestination(
    text.value,
    (keyword) => `/specialAreas?keyword=${encodeURIComponent(keyword)}`
  )
