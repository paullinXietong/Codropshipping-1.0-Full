          ...
          <div v-for="(it,nt) in list" :key="nt" class="flex flex-col sm:flex-row p-4 bg-white mb-2">
            <div class="flex-shrink-0 flex">
              ...
              <div class="block sm:hidden">
                <a :href="'/productDetail?id='+it.goods_id+'&source='+it.source"
            ...
            <div class="w-full">
              <div class="flex items-center">
                <img v-show="it.source < 4" src="@/assets/1688.png" class="w-8 h-8 mr-2" alt="">
                <img v-show="it.source == 5" src="@/assets/alibaba.png" class="h-8 mr-2" alt="">
                <a :href="'/productDetail?id='+it.goods_id+'&source='+it.source"
                  target="_blank" class="hover:underline">
    ...
    methods: {
      ...
      checkSku(item,list){
        ...
        for(var n=0; n<list.length; n++){
          for(var m=0; m<item.sku_info.length; m++){
            if(list[n] == item.sku_info[m].id){
              sku_num += Number(item.sku_info[m].num)
              sku_price += Number(item.sku_info[m].total)
            }
          }
        }
        item.qtyTotal = sku_num
        item.priceTotal = sku_price.toFixed(2)
        
        for(var i=0; i<this.list.length; i++){
          total_num += Number(this.list[i].qtyTotal)
          total_price += Number(this.list[i].priceTotal)
        }
        this.totalNum = total_num
        this.totalPrice = total_price.toFixed(2)
      },
