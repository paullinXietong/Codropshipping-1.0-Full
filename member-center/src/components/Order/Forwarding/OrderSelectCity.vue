    ...
    <div ref="selectUl"
      ...
      <ul>
        <li :ref="item.e_name"
            ...
            :class="{ checked: selectChecked.e_name == item.e_name}"
            v-for="(item, index) in list"
            :key="index"
            @click="CheckedOne(item)">{{item.e_name}}</li>
      </ul>
    </div>
  </div>
</template>
    ...
    selectOpen: {
      handler(val, oldVal) {
        ...
        if (this.list.length > 0) {
          if (this.$refs[this.selectChecked.e_name][0].offsetTop > 240) {
            this.$refs.selectUl.scrollTop =
              this.$refs[this.selectChecked.e_name][0].offsetTop - 200
          } else {
  ...
  methods: {
    ...
    search() {
      ...
      for (let n = 0; n < this.selectList.length; n++) {
        if (this.selectList[n].e_name.toUpperCase().indexOf(str) != -1) {
