...
member_dir="$project_dir/member-center"
storefront_dir="$project_dir/web-search"
search_dir="$storefront_dir/search-backend"

for required_dir in "$member_dir/node_modules" "$storefront_dir/node_modules" "$search_dir/node_modules"; do
...
(cd "$member_dir" && npm run serve -- --host 127.0.0.1 --port 8083) &
demo_pids+=($!)
