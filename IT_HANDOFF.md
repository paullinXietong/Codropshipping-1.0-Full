...
2. 测试域名及正确 DNS；Caddy 会申请 HTTPS 证书。
...
| 服务 | 内部职责 | 公网路径 |
| --- | --- | --- |
| `web` | 官网、浏览、搜索、商品详情、普通 List / AI List | `/` |
| `member-center` | 总览、商品库、销售渠道、刊登、订单、履约、库存、财务、洞察 | `/admin/*` |
| `listing-api` | 草稿、AI 文案/翻译/图片、Shopify 发布编排 | `/listing-api/*` |
| `search-api` | 商品搜索、筛选、索引及上游降级 | `/search-api/*` |
| `caddy` | HTTPS、同源路由、现有 CoD API 安全代理 | `80/443` |
